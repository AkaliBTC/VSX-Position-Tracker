...
const NEW_TTL = 24 * 60 * 60 * 1000; // 24h in ms
...
const fetchBinance = async (ticker) => {
  ...
  try {
    ...
    if (res.ok) { const price = parseFloat((await res.json()).price); if (price > 0) return price; }
  } catch {}
  try {
...
const fetchYahooSingle = async (ticker) => {
  ...
  for (const [url, extract] of pairs) {
    for (const px of PROXIES) {
      try { const data = await px(url); const price = extract(data); if (price && price > 0) return price; }
      catch { continue; }
    }
  }
  return null;
...
const fetchYahooBatch = async (tickers) => {
  ...
  for (const chunk of chunks) {
    ...
    for (const base of ["query1", "query2"]) {
      ...
      for (const px of PROXIES) {
        try { const data = await px(url); const res = data?.quoteResponse?.result; if (res?.length) { items = res; break; } }
        catch { continue; }
        if (items) break;
      }
      if (items) break;
    ...
    if (items) {
      ...
      for (const ticker of chunk) results[ticker] = await fetchYahooSingle(ticker);
    }
  }
  return results;
};
  ...
  return (
    <div>
      ...
      <div className="toolbar">
        ...
        <input className="search-inp" placeholder="Search ticker, date, direction…"
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {sorted.length === 0 ? (
              <tr><td colSpan={12} className="empty-cell">
                {search ? "NO RESULTS" : "NO POSITIONS — PRESS ADD TO BEGIN"}
              </td></tr>
            ) : sorted.map((p) => {
              ...
              return (
                <tr key={p.id}
                  ...
                  {/* QTY — between direction and entry */}
                  <td>
                    <input className="cell-input num-inp qty-inp" placeholder="0" type="number"
                      value={p.qty} onChange={(e) => update(p.id, "qty", e.target.value)}
                      onFocus={() => setFocus(p.id)} onBlur={() => clearFocus()} />
                  </td>
                  <td>
...
export default function App() {
  ...
  // Prune expired NEW flags every minute
  useEffect(() => {
    const t = setInterval(() => {
  ...
  useEffect(() => {
    const intervals = TABS.map((tab) => {
      ...
      return setInterval(() => {
        if (!anyFocused.current && (allPositions[tab.id] || []).some((p) => p.ticker.trim()))
          refreshTab(tab.id);
      }, ms);
    });
    ...
    <div className="app">
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-label { font-family: 'Montserrat', sans-serif; font-size: 8px; font-weight: 600; letter-spacing: 0.22em; color: var(--text-dim); text-transform: uppercase; margin-bottom: 4px; }
        ...
        .live-badge { display: flex; align-items: center; gap: 7px; padding: 5px 14px; border: 1px solid rgba(34,197,94,0.2); background: rgba(34,197,94,0.06); border-radius: 20px; font-family: 'Montserrat', sans-serif; font-size: 8px; font-weight: 600; letter-spacing: 0.22em; color: var(--green); }
        ...
        .save-flash { font-size: 8px; letter-spacing: 0.18em; color: var(--gold2); transition: opacity 0.4s; font-family: 'Montserrat', sans-serif; font-weight: 500; }
        .save-flash.on { opacity: 1; } .save-flash.off { opacity: 0; }
        .refresh-ts { font-size: 9px; color: var(--text-mute); letter-spacing: 0.06em; }
        .new-count-badge { font-family: 'Montserrat', sans-serif; font-size: 9px; font-weight: 700; letter-spacing: 0.14em; padding: 3px 10px; border-radius: 20px; background: rgba(212,175,55,0.12); color: var(--gold2); border: 1px solid rgba(212,175,55,0.3); }

        .tabs-wrap { display: flex; background: var(--black); border-bottom: 1px solid var(--border); padding: 0 56px; gap: 4px; }
        .tab { padding: 18px 20px; font-family: 'Montserrat', sans-serif; font-size: 10px; font-weight: 600; letter-spacing: 0.18em; text-transform: uppercase; color: var(--text-dim); cursor: pointer; border: none; background: transparent; border-bottom: 1px solid transparent; transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1); position: relative; bottom: -1px; display: flex; align-items: center; gap: 9px; }
        ...
        .hint-label { font-family: 'Montserrat', sans-serif; font-size: 8px; font-weight: 700; letter-spacing: 0.25em; color: var(--gold2); white-space: nowrap; padding-right: 20px; border-right: 1px solid var(--border); }

        .toolbar { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; }
        .btn { padding: 10px 22px; font-family: 'Montserrat', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.15em; border: none; cursor: pointer; transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1); text-transform: uppercase; border-radius: 6px; }
        ...
        .search-inp { background: var(--black2); border: 1px solid var(--border); color: var(--text); font-family: 'DM Mono', monospace; font-size: 11px; padding: 8px 14px; border-radius: 6px; outline: none; width: 220px; letter-spacing: 0.04em; transition: border-color 0.2s, background 0.2s; }
        .search-inp:focus { border-color: var(--gold1); background: rgba(212,175,55,0.04); }
        .search-inp::placeholder { color: var(--text-mute); }
        .source-badge { font-size: 9px; color: var(--text-mute); letter-spacing: 0.12em; font-family: 'Montserrat', sans-serif; font-weight: 500; margin-left: 4px; }

        .table-wrap { border: 1px solid var(--border); overflow-x: auto; background: var(--black2); border-radius: 12px; overflow: hidden; transition: border-color 0.3s; }
        .table-wrap:hover { border-color: var(--border2); }
        table { width: 100%; border-collapse: collapse; min-width: 1100px; }
        thead tr { background: var(--black3); border-bottom: 1px solid var(--border); }
        th { padding: 14px 12px; font-family: 'Montserrat', sans-serif; font-size: 8px; font-weight: 700; letter-spacing: 0.28em; color: var(--text-dim); text-align: left; white-space: nowrap; }
        th:first-child { color: var(--gold1); }
        tbody tr { border-bottom: 1px solid var(--border); transition: background 0.2s; cursor: default; }
        ...
        .num-inp { width: 85px; }
        .qty-inp { color: var(--gold3); width: 75px; }
        .date-inp { width: 130px; color-scheme: dark; }

        .dir-sel { border: none; font-family: 'Montserrat', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.15em; cursor: pointer; padding: 5px 12px; outline: none; -webkit-appearance: none; text-transform: uppercase; border-radius: 4px; transition: all 0.2s; }
        ...
        .del-btn { background: none; border: none; color: var(--text-mute); cursor: pointer; font-size: 12px; padding: 6px 10px; transition: all 0.2s; border-radius: 4px; }
        .del-btn:hover { color: var(--red); background: rgba(239,68,68,0.08); transform: scale(1.1); }
        .empty-cell { text-align: center; padding: 72px; font-family: 'Montserrat', sans-serif; color: var(--text-mute); font-size: 10px; letter-spacing: 0.3em; font-weight: 500; }
        ...
        {TABS.map((t) => {
          ...
          return (
            <button key={t.id} className={`tab ${activeTab === t.id ? "active" : ""}`} onClick={() => setActiveTab(t.id)}>
              ...
              {tabNew > 0 && (
                <span style={{ fontSize: 8, padding: "1px 6px", borderRadius: 20, background: "rgba(212,175,55,0.15)", color: "var(--gold2)", border: "1px solid rgba(212,175,55,0.3)", letterSpacing: "0.1em", fontWeight: 700 }}>
                  {tabNew}N
                </span>
              )}
